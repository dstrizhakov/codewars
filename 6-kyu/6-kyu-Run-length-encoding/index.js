...
var runLengthEncoding = function (str) {
  // разбивает на строку из массив построк
  // где каждая подстрока это повторяющиеся символы
  let arr = str.match(/(.)\1*/g) || [];
  // Возвращаем новый массив построенный на основе старого
  // состоящий из длинны подстроки и нулевого символа этой подстроки
  let result = arr.map((c) => [c.length, c[0]]);
  return result;
};
